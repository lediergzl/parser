...
function crearLockSesionWhatsapp(supabase, lockId = 'default') {
  ...
  async function intentarAdquirir() {
    ...
    const adquirido = Boolean(data);
    if (!adquirido && sostenido) {
      sostenido = false;
      if (!perdidaNotificada) {
        perdidaNotificada = true;
        try { onLost?.(); } catch (err) {
          console.error('⚠️ Error ejecutando onLost del lock WhatsApp:', err?.message || err);
        }
      }
    } else if (adquirido) {
      sostenido = true;
      perdidaNotificada = false;
    }
    return adquirido;
  }
  ...
  function setOnLost(callback) {
    onLost = typeof callback === 'function' ? callback : null;
  }

  return { esperarYAdquirir, liberar, setOnLost, holderId: HOLDER_ID };
}
