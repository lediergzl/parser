...
async function sendNative(sock, jid, bodyText, buttons, fallbackText = bodyText, interactiveJid = null) {
  ...
  // WhatsApp entrega el chat entrante como @lid, pero también nos entrega
  // senderPn con el JID real del teléfono. El native-flow debe construirse y
  // relayedarse al JID de teléfono; responder al @lid sigue siendo correcto
  // para los mensajes normales.
  ...
  try {
    ...
    // WhiskeySockets no añade automáticamente los nodos binarios que
    // WhatsApp espera para native-flow. Sin ellos, relayMessage acepta el
    // mensaje pero el cliente receptor puede ignorarlo silenciosamente.
