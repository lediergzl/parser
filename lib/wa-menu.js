...
async function sendNative(sock, jid, bodyText, buttons, fallbackText = bodyText, interactiveJid = null) {
  ...
  if (!replyJid) return false;

  // IMPORTANTE:
  // El JID de respuesta es el remoteJid real del mensaje recibido. En chats
  // con LID, senderPn puede ser distinto del remoteJid. Nunca debemos cambiar
  // el destino del menú interactivo al PN del remitente: WhatsApp puede aceptar
  // el relay pero el cliente queda mostrando "Esperando el mensaje".
  //
  // interactiveJid se conserva solo como información de diagnóstico/compatibilidad;
  // el mensaje SIEMPRE se genera y se relaya al chat que originó la conversación.
  ...
  try {
    ...
    // Estructura estándar: interactiveMessage dentro de viewOnceMessage,
    // con messageContextInfo para compatibilidad multidispositivo.
    ...
    console.log('[WA MENU] interactivo enviado', {
      target,
      remoteJid: replyJid,
      interactiveJid: interactiveJid || null,
      id: fullMsg.key.id,
      privateChat,
      firstButtonName
    });
    ...
    console.error('[WA MENU] ERROR interactivo target=' + target + ':', error?.stack || error?.message || error);

    // El fallback solo se utiliza si la construcción/entrega del interactivo
    // realmente lanza una excepción. No mandamos menú plano junto al interactivo:
    // eso duplicaría el flujo y podría dejar dos estados de conversación.
    try {
      await sock.sendMessage(replyJid, { text: fallbackText });
      console.log('[WA MENU] fallback texto enviado a ' + replyJid);
    } catch (fallbackError) {
