...
// El menú inicial de /jugar se envía como texto normal para garantizar que el cliente lo vea.
async function sendMainMenu(sock, jid) {
  ...
  try {
    ...
    console.log(`[WA MENU] menú principal visible enviado a ${jid}`);
    return true;
  } catch (error) {
    console.error(`[WA MENU] ERROR enviando menú principal a ${jid}:`, error?.stack || error?.message || error);
    return false;
  }
}
