...
async function sendNative(sock, jid, bodyText, buttons, fallbackText = bodyText) {
  ...
  // Los mensajes interactivos (nativeFlowMessage) no se renderizan en
  // WhatsApp cuando el destino es un JID @lid (confirmado: relayMessage no
  // tira error pero el cliente nunca ve el menú). Intentar resolverlo al
  // número de teléfono corrompe la sesión de cifrado Signal (ver commit
  // f0f2e64). Hasta que Baileys soporte esto correctamente, para @lid
  // vamos directo al texto plano, que sí llega.
  if (target.endsWith('@lid')) {
    try {
      await sock.sendMessage(target, { text: fallbackText });
      return true;
    } catch (error) {
      console.error(`[WA MENU] ERROR enviando texto a ${target}:`, error?.stack || error?.message || error);
      return false;
    }
  }
  ...
  try {
    ...
    await sock.relayMessage(target, generated.message, { messageId: generated.key.id });
    console.log(`[WA MENU] interactivo enviado a ${target}`);
    return true;
  } catch (error) {
    console.error(`[WA MENU] ERROR para ${target}:`, error?.stack || error?.message || error);

    try {
      await sock.sendMessage(target, { text: fallbackText });
      console.log(`[WA MENU] fallback texto enviado a ${target}`);
    } catch (fallbackError) {
