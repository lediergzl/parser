...
async function useSupabaseAuthState(supabase, sessionId = 'default') {
  ...
  async function readRow() {
    const { data, error } = await supabase.from(table).select('creds,keys').eq(column, id).maybeSingle();
  ...
  async function persist() {
    const payload = commercial
      ? { comercial_telegram_id: id, creds: serialize(creds), keys: serialize(keysData), updated_at: new Date().toISOString() }
      : { id, creds: serialize(creds), keys: serialize(keysData), updated_at: new Date().toISOString() };
