...
async function leerGrupoResultadosComercial(supabase, comercialId) {
  const { data, error } = await supabase
    .from('whatsapp_comercial_resultados')
    .select('comercial_telegram_id,destino_id,nombre,activo,updated_at')
...
async function guardarGrupoResultadosComercial(supabase, comercialId, destinoId, nombre) {
  const grupo = normalizarGrupo(destinoId);
  if (!grupo) throw new Error('El mensaje debe estar dentro de un grupo de WhatsApp válido.');
  const { data, error } = await supabase
    .from('whatsapp_comercial_resultados')
    .upsert({
...
async function listarGruposResultadosComerciales(supabase) {
  const { data, error } = await supabase
    .from('whatsapp_comercial_resultados')
    .select('comercial_telegram_id,destino_id,nombre,activo,updated_at')
    .eq('activo', true)
    .order('comercial_telegram_id');
  if (error) throw error;
  return data || [];
}
...
module.exports = {
  ...
  listarGruposResultadosComerciales,
  leerGrupoResultadosComercial,
  guardarGrupoResultadosComercial,
