...
const { listarGruposResultados } = require('./whatsapp-destino');
...
async function conectarWhatsapp(supabase) {
  ...
  if (!suscrito) {
    ...
    bus.on('resultado:recibido', async (resultado) => {
      ...
      try {
        grupos = (await listarGruposResultados(supabase)).filter(g => g?.activo && g?.destino_id);
      } catch (error) {
        console.error('❌ No se pudo cargar la lista de grupos autorizados para resultados:', error?.message || error);
        return;
      }

      if (!grupos.length) {
        console.log('ℹ️ Resultado recibido, pero no hay grupos WhatsApp autorizados para publicarlo.');
        return;
      }
      ...
      for (const grupo of grupos) {
        ...
        try {
          // Si WhatsApp está desconectado, enviarNotificacionWhatsapp lo deja
          // en cola para entregarlo cuando vuelva a estar OPEN.
          const ok = await enviarNotificacionWhatsapp(
            destinoId,
            texto,
            `Resultado #${resultado?.resultadoId ?? '-'} → ${grupo.nombre || destinoId}`
          );
          if (ok) enviados++;
        } catch (error) {
      ...
      console.log(`📢 Resultado #${resultado?.resultadoId ?? '-'} procesado para ${grupos.length} grupo(s) autorizado(s); enviados ahora: ${enviados}.`);
    });
