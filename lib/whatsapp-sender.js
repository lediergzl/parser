...
function qrTerminalToPng(qrText, scale = 8) {
  ...
  let rendered = '';
  qrcode.generate(qrText, { small: false }, output => { rendered = output; });
  if (!rendered) throw new Error('qrcode-terminal no devolvió una representación del QR.');

  // qrcode-terminal 0.12.0 en modo grande representa cada módulo con una
  // celda ANSI: fondo negro (40) o blanco (47). Convertimos esas celdas a una
  // matriz binaria y luego a PNG. El token QR nunca sale del servidor.
  const rows = [];
  const cellRegex = /\x1b\[(40|47)m \x1b\[0m/g;
  for (const line of String(rendered).split('\n')) {
    const cells = [];
    let match;
    while ((match = cellRegex.exec(line)) !== null) {
      cells.push(match[1] === '40');
    }
    cellRegex.lastIndex = 0;
    if (cells.length) rows.push(cells);
  }

  if (!rows.length) throw new Error('No se pudieron extraer módulos del QR generado.');
  const width = rows[0].length;
  if (!width || rows.some(row => row.length !== width)) {
    throw new Error('La matriz QR generada tiene filas inconsistentes.');
  }
  ...
  for (let moduleY = -quiet; moduleY < rows.length + quiet; moduleY++) {
    ...
    for (let pixelX = 0; pixelX < imageSize; pixelX++) {
      ...
      const dark = moduleY >= 0 && moduleY < rows.length && moduleX >= 0 && moduleX < width
        ? rows[moduleY][moduleX]
        : false;
    ...
    // Cada módulo se amplía verticalmente 'scale' veces.
