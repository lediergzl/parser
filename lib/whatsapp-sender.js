...
async function guardarNotificacionOutbox(supabase, { tipo, referenciaId, destinoId, texto, etiqueta }) {
  ...
  const { error } = await supabase
    ...
    .upsert([fila], {
      onConflict: 'tipo,referencia_id,destino_id',
...
async function enviarOutboxPorSocketOTransport(supabase, destinoId, texto) {
  if (ready && sock) {
  ...
  vaciadoOutboxEnCurso = (async () => {
    ...
    try {
      const { data: pendientes, error } = await supabase
        .from('whatsapp_notificaciones_outbox')
        .select('id,tipo,referencia_id,destino_id,texto,etiqueta,intentos')
        .eq('estado', 'pendiente')
      ...
      for (const item of pendientes || []) {
        ...
        try {
          await enviarOutboxPorSocketOTransport(supabase, String(item.destino_id), item.texto);
          ...
          console.error(`❌ Error enviando ${item.etiqueta || item.tipo + ' #' + item.referencia_id} a WhatsApp:`, sendError?.message || sendError);

          await supabase
            .from('whatsapp_notificaciones_outbox')
            .update({
              ultimo_error: String(sendError?.message || sendError).slice(0, 1000)
            })
            .eq('id', item.id);
...
async function probarResultadoWhatsapp(supabase, comercialId) {
  const id = Number(comercialId);
  if (!Number.isFinite(id)) throw new Error('comercialId inválido.');

  const grupo = await leerGrupoResultadosComercial(supabase, id);
...
async function encolarNotificacionWhatsapp(supabase, tipo, referenciaId, destinoId, texto, etiqueta) {
  const guardada = await guardarNotificacionOutbox(supabase, {
    tipo, referenciaId, destinoId, texto, etiqueta
  });
    ...
    bus.on('resultado:recibido', async (resultado) => {
      ...
      for (const [destinoId, meta] of destinos) {
        await encolarNotificacionWhatsapp(
          supabase,
          'resultado',
          // La clave incluye los números: una corrección del mismo resultado se
          // envía, pero el mismo resultado no se duplica.
          `${resultado?.resultadoId}:${resultado?.fijo ?? ''}:${Array.isArray(resultado?.corrido) ? resultado.corrido.join('-') : ''}:${resultado?.centena ?? ''}`,
          destinoId,
          texto,
