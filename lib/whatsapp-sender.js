...
async function enviarPhotoDirectoTelegram(adminId, buffer, caption) {
  ...
  try {
    ...
    if (!response.ok || !data.ok) {
      throw new Error(
        `Telegram sendPhoto HTTP ${response.status}: ${data.description || 'respuesta no válida'}`
      );
    }
    ...
    if (error?.name === 'AbortError') {
      throw new Error('Telegram sendPhoto agotó el tiempo de espera (20 s).');
    }
    throw error;
  } finally {
...
async function enviarPhotoConReintento(adminId, buffer, caption, maxIntentos = 3) {
  ...
  for (let intento = 1; intento <= maxIntentos; intento++) {
    try {
      ...
      ultimoError = error;
      console.error(
        `⚠️ Fallo enviando QR al admin ${adminId} (intento ${intento}/${maxIntentos}):`,
        error?.message || error
      );
  ...
  console.error(
    `❌ No se pudo enviar el QR al admin ${adminId} tras ${maxIntentos} intentos:`,
...
function encolarEnvioQr(qr) {
  // Evita que varios QR intenten subir imágenes simultáneamente cuando Baileys
  // emite renovaciones rápidas del QR.
