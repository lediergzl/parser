...
function formatearResultado(r) {
  ...
  return [
    '🎲 RESULTADO RECIBIDO',
    '',
    `🎰 Lotería: ${r.loteriaNombre || `#${r.loteriaId ?? '-'}`}`,
    `🕒 Sorteo: ${r.nombreSorteo || `#${r.sorteoId ?? '-'}`}`,
    `📅 Fecha: ${r.fecha || '-'}`,
    '',
    `🔢 Fijo: ${r.fijo || '—'}`,
    `🎯 Corridos: ${corrido}`,
...
async function enviarPhotoDirectoTelegram(adminId, buffer, caption) {
  ...
  try {
    ...
    try {
      ...
      throw new Error(`Telegram devolvió HTTP ${response.status} sin JSON válido.`);
    }

    if (!response.ok || !data.ok) {
      throw new Error(`Telegram sendPhoto HTTP ${response.status}: ${data.description || 'respuesta no válida'}`);
    }
    return data;
  } catch (error) {
...
async function enviarPhotoConReintento(adminId, buffer, caption, maxIntentos = 3) {
  ...
  for (let intento = 1; intento <= maxIntentos; intento++) {
    try {
      await enviarPhotoDirectoTelegram(adminId, buffer, caption);
      return true;
    } catch (error) {
      ultimoError = error;
...
async function enviarQrAlAdministrador(qr) {
  ...
  try {
    ...
    for (const adminId of adminIds) {
      const ok = await enviarPhotoConReintento(adminId, buffer, caption);
      if (ok) enviados++;
    }

    if (enviados > 0) {
      console.log(`✅ QR de WhatsApp enviado por Telegram a ${enviados}/${adminIds.length} administrador(es).`);
      return true;
    }
...
function encolarEnvioQr(qr) {
  envioQrEnCurso = envioQrEnCurso
    .catch(() => {})
    .then(() => enviarQrAlAdministrador(qr));
...
async function enviarNotificacionWhatsapp(destinoId, texto, etiqueta) {
  ...
  if (!ready || !sock) {
    if (colaNotificaciones.length < 100) {
      ...
      console.error(`❌ Cola de WhatsApp llena; se descartó notificación ${etiqueta}.`);
    }
    return false;
  }
...
async function vaciarColaNotificacionesWhatsapp() {
  ...
  while (ready && sock && colaNotificaciones.length) {
    ...
    if (!ok && !ready) {
      colaNotificaciones.unshift(item);
      break;
    }
  }
}
...
async function conectarWhatsapp(supabase) {
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'open') {
      ...
      vaciarColaNotificacionesWhatsapp().catch(err =>
        console.error('❌ Error vaciando cola de notificaciones WhatsApp:', err));
    }
