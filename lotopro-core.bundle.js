...
function _detectarCentenaGlobal(l) {
  ...
  if (mInline) {
    ...
    const expandidos = [];
    for (const c of centenas) {
      for (const base of bases) expandidos.push(c + base);
    }
    return 'INLINE:' + expandidos.join(' ') + ' con ' + monto;
  }
  ...
  // porque la 'x' sin dígitos adyacentes es eliminada como token inválido.
  {
    ...
    if (cgEarly !== null) {
      ...
      if (cgEarly.startsWith('INLINE:')) {
        ...
        trace('PRE_FILTERED', {
          id, rawLine, resultado: inlineLine,
          razon: 'instrucción xc inline expandida a centenas por números base',
        });
        return inlineLine;
      }
